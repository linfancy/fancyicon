...
function init () {
    ...
    new Vue({
        ...
        methods: {
             ...
             */
            handleDelIcon (e) {
                const index = e.detail.dataset && e.detail.dataset.index ? parseInt(e.detail.dataset.index) : '';
            ...
            handleEditCode (e) {
                ...
                const index = p.getAttribute('data-num') ? parseInt(p.getAttribute('data-num')) : '';
                if (index !== '') {
                    const tempSvgUnicode = '\\' + this.info.glyphs[index].code.toString(16);
            ...
            _createConfig () {
                ...
                return new Promise((resolve, reject) => {
                    fs.writeFile(
                        path.join(that.info.configPath),
                        JSON.stringify(that.info),
                        function (err) {
                            if (err) reject(err);
                            else resolve();
                        }
