...
function processTree (node, ignoredTags, ignoredAttrs, parentTransforms, path) {
    ...
    // If item not Node - skip node . Example: #text - text node, comments
        ...
        const transforms = (item.getAttribute('transform')) ? parentTransforms + ' ' + item.getAttribute('transform') : parentTransforms;
        // Parse nested tags
        if (item.nodeName === 'g') {
            const result = processTree(item, ignoredTags, ignoredAttrs, transforms, path);
            path = result.path;
        ...
        const transformedPath = new SvgPath(d).transform(transforms).toString();
...
function getCoordinates (svg) {
    ...
    _.forEach(['x', 'y', 'width', 'height'], function (key) {
        const val = svg.getAttribute(key);
 ...
 */
export default function convert (sourceXml) {
    let error = null;
    const result = {
        d: '',
        width: 0,
    ...
    const processed = processTree(svg, {}, {}, '', '');

    const coords = getCoordinates(svg);
